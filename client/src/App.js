...
const App = () => {
  ...
  useEffect(() => {
    if (web3) {
      if (window.ethereum) {
        ...
        console.log(
          "Fallback to localhost detected - subscription to account change is being ignored now."
        );
      }
    }
  }, [web3]);
    ...
    const createItemManager = async () => {
      try {
        const instance = new web3.eth.Contract(
          ...
          ItemManagerContract.networks[await web3.eth.net.getId()] &&
            ItemManagerContract.networks[await web3.eth.net.getId()].address
        );
  ...
  useEffect(() => {
    if (itemManager) {
      ...
      itemManager.events.ItemPaid().on("data", (event) => {
        console.log(
          `Item of ID: ${event.returnValues._itemIndex} at address ${event.returnValues._itemAddress} has been paid for.`
        );
      });
      itemManager.events.ItemDelivery().on("data", (event) => {
        console.log(
          `Item of ID: ${event.returnValues._itemIndex} at address ${event.returnValues._itemAddress} is being delivered now.`
        );
      });
    }
  ...
  const handleItemDelivery = async () => {
    ...
    try {
      await itemManager.methods
        .triggerDelivery(selectedItemIndex)
        .send({ from: from });
    } catch (error) {
      console.error(error);
    } finally {
  ...
  return (
    <Grid container spacing={4} justifyContent="center" padding={"0 32px"}>
      <Grid item xs={12}>
        <Typography
          variant="h2"
          component="h1"
          ...
          currentAccount !== "" ? (
            ...
            >
              <Typography
                ...
                paddingRight={1}
              >
                Connected succefully to address {currentAccount}
              </Typography>
              <Jazzicon
                diameter={30}
            ...
            <Typography align="center" variant="body1">
              Web3 detected - make sure to connect your account
            </Typography>
          )
        ) : (
          <Grid item xs={12}>
            {" "}
            <Typography align="center" variant="body1">
              Loading Web3, accounts, and contract... Please make sure your
      ...
      <Grid item>
        <Stack
          boxShadow={"#5f5f5f50 12px 12px 54px"}
          padding={2}
          marginBottom={8}
        >
          <Stack direction={"row"} marginBottom={2} justifyContent="center">
            ...
            <Container >
              {item ? (
                ...
                <Typography align="center" variant="body1">
                  Select item to perform any action.
                </Typography>
              )}
            </Container>
          </Stack>
          ...
          <Box>
            ...
            <ItemTable items={items} handleItemChange={handleItemChange} />
          </Box>
        </Stack>
      </Grid>
    </Grid>
